...
交易日历保存在 MySQL 的 `trading_calendar` 表中。每次判断交易日时优先读取数据库，不在 monitor 进程内缓存；仅当数据库缺少对应年份时才从 AKShare 获取并回写数据库。策略配置保存在 `strategy_definitions`、`strategy_profiles`、`strategy_change_log` 等表中，策略中心中的保存操作先生成待确认版本，确认后由下一轮 monitor 动态读取。
